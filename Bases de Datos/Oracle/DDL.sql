...
CREATE OR REPLACE PROCEDURE DROP_TABLAS
IS
CURSOR TABLAS IS SELECT table_name FROM user_tables;
CURSOR SECUENCIAS IS SELECT sequence_name FROM user_sequences;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('DROP A LA TABLA ' || REGISTRO);
    END LOOP;
    CLOSE TABLAS;
    
    OPEN SECUENCIAS;
    LOOP
    FETCH SECUENCIAS INTO REGISTRO;
    EXIT WHEN SECUENCIAS%NOTFOUND;
    EXECUTE IMMEDIATE ('DROP SEQUENCE ' || REGISTRO);
    DBMS_OUTPUT.PUT_LINE('DROP A LA SECUENCIA ' || REGISTRO);
    END LOOP;
    CLOSE SECUENCIAS;
END;
/
